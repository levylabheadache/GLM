function [frames, dilation] = glmOnsetsBehavioral(licking, ensure, quinine, framerate, nframes, dilate, behavior)
...
%   behavior name from a simpcell file.
    ...
    if length(name) == 1
        ...
        elseif strcmpi(name{1}, 'quinine')
            frames = quinine;
        elseif strcmpi(name{1}, 'outcome')
            frames = sort([ensure quinine]);
        end
    else
